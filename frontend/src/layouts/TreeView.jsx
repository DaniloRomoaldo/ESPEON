...
import SchemaIcon from "../components/treeView/Schema";
import ExpansionButton from "../components/treeView/ExpansionButton";
import TableIcon from "../components/treeView/Tables";
import ViewsIcon from "../components/treeView/Views";
import FunctionsIcon from "../components/treeView/Functions";
import TriggersIcon from "../components/treeView/Triggers";
import ProceduresIcon from "../components/treeView/Procedures";
import EnumsIcon from "../components/treeView/Enums";
import { getDatabaseElement } from "../api/databaseElements";
import TreeSection from "../components/treeView/TreeSection";
...
export default function Treeview() {
  ...
  useEffect(() => {
    const hasData =
      tableList.length > 0 || viewsList.length > 0 || functionsList.length > 0;
  ...
  return (
    <div className="h-full w-full overflow-y-auto p-2 overflow-scroll bg-[#131315]">
      <div
        className="hs-accordion-treeview-root overflow-y-auto overflow-x-auto w-full"
        role="tree"
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div
                className="hs-accordion-group ps-7 overflow-y-auto overflow-x-auto w-full relative before:absolute before:top-0 before:start-3 before:w-0.5 before:-ms-px before:h-full before:bg-gray-100 dark:before:bg-neutral-700"
                role="group"
                data-hs-accordion-always-open=""
              >
                {sections.map((section) => (
                  <TreeSection
                    ...
                    list={section.list}
                    onExpand={() =>
                      handleExpand(section.elementName, section.hookName)
                    }
                  />
                ))}
              </div>
