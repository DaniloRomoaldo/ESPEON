...
export async function startLab(exercieListId) {
  try {
    ...
    throw new Error(
      error.response?.data?.error || "Não foi possível iniciar o ambiente"
    );
  }
}
...
export async function stopLab(labSessionId) {
  try {
    ...
    throw new Error(
      error.response?.data?.error || "Não foi possível parar o ambiente"
    );
  }
}
...
export async function checkAnswer(body) {
  try {
    ...
    throw new Error(
      error.response?.data?.error || "Não foi possível verificar a resposta."
    );
  }
}
