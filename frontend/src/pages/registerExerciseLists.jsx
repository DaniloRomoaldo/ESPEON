...
import FileUpload from "../components/exerciseLists/FileUpload.jsx";
import ExerciseForm from "../components/exerciseLists/ExerciseForm.jsx";
import { useState, useRef } from "react";
import registerExerciseList from "../api/exerciseLists.js";
import Cookies from "js-cookie";

import HamburguerMenu from "../components/HamburguerMenu";
import DrawerMenu from "../components/DrawerMenu";

export default function RegisterExerciseLists() {
  ...
  const addForm = () => {
    ...
    setExercises((prev) => [
      ...prev,
      { name: "", description: "", solution_query: "" },
    ]);
  };
  ...
  async function handleSubmit(e) {
    ...
    const isAnyExerciseEmpty = exercises.some(
      (ex) => !ex.name || !ex.description || !ex.solution_query
    );
    if (!listName || !uploadedFile || isAnyExerciseEmpty) {
      alert(
        "Por favor, preencha todos os campos obrigatórios antes de submeter."
      );
      return;
    }
  ...
  return (
    <>
      <div className="relative h-screen flex flex-col items-center  p-4 sm:p-6 md:p-8">
        ...
        <form
          key={resetKey}
          className="w-[60%] h-[95%] bg-[#202024] flex flex-col"
        ...
        >
          <div className="w-full px-[3%] py-4">
            <div>
              <label
                htmlFor="list_name"
                className="block text-xs sm:text-sm text-white opacity-30 font-extralight mt-2"
              >
                Lista de Exercícios
              </label>
