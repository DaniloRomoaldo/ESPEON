...
export const create = async (body, query) => {
  ...
  if (listExist[0]) {
    //devolver um array de objetos
    throw new Error(
      "Existe uma lista de exercício com esse nome, por favor escolha outro nome"
    );
  }
...
export const deleteExerciseList = async (list_id, user_id) => {
  ...
  if (!exercices_list[0]) throw new Error("Lista de exercícos não encontrada");
    ...
    exercices_list[0].db_path && typeof exercices_list[0].db_path === "string";
